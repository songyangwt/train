...
var arry= new Array();
arry=[year,year-1,year-2,year-3,year-4,year-5,year-6,year-7,year-8,year-9];
...
for (var i=0;i<arry.length;i++){	
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  ${daohang}
  <form action="<%=path%>/viewchuplanyear.action" method="post" name="fm1">
						...
						<tr height="50px" class="表格表头背景1" id="hang">
							...
							<tr class="btbj" id="hang" style="height:20px">
								...
								<td width="140px" height="25" align="center" valign="middle" nowrap><div
										...
										&nbsp;&nbsp;
										 -->
										<a href="<%=path%>/showtrainplanmodify.action?id=${tp.id}">修改</a>
										<a href="<%=path%>/trainplandelete.action?id=${tp.id}">删除</a>
										</div></td>	
							</tr>
							</c:forEach>
							<tr class="表格表头背景">
							<td colspan="13">
								<div align="center">
								<a	href="<%=path%>/viewchuplanyear.action?year=${year}&newnumber=${newnumber}&currentPage=${previousPage}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/viewchuplanyear.action?year=${year}&newnumber=${newnumber}&currentPage=${nextPage}"
