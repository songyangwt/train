...
$("tr.btbj:even").css({"background-color":"#bfd3fc","font-family": "黑体","font-size": "14px","font-weight":"lighter" }); 
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  ${daohang}
  <form action="<%=path%>/viewtrainapplyunder.action" method="post" name="fm1">
					...
					<tr height="50px" class="表格表头背景1" id="hang">
							...
							<tr class="btbj" id="hang" style="height:20px">
								...
								<td width="140px" height="25" align="center" valign="middle" nowrap><div
										...
										<c:if test="${(newnumber==pxsq.initiator)&&(pxsq.status==0||pxsq.status==5)}">
												<a href="<%=path%>/deletetrainapplypage.action?number=${pxsq.number}&newnumber=${newnumber}">撤销</a>&nbsp;&nbsp;
												<a href="<%=path%>/viewtrainapplydetail.action?number=${pxsq.number}&newnumber=${newnumber}">查看详情</a> 
										</c:if>
                                         <c:if test="${pxsq.status==1}">
                                               <a href="<%=path%>/viewtrainapplyunderdetail.action?number=${pxsq.number}">处理</a>
                                         </c:if>
                                          <c:if test="${pxsq.status==2}">
                                              <a href="<%=path%>/showtrainapplynotice.action?number=${pxsq.number}&newnumber=${newnumber}">发布培训公告</a>
                                          </c:if>
                                          <c:if test="${pxsq.status==3&&pxsq.isregister==1}">
										  <a href="<%=path%>/maketrainsheet.action?number=${pxsq.number}&newnumber=${newnumber}">生成签到表</a>
										  </c:if>
										    <c:if test="${pxsq.status==3&&pxsq.isregister==2}">
										  <a href="<%=path%>/issuescore.action?number=${pxsq.number}&newnumber=${newnumber}">发布评估信息</a>
										  </c:if>
									      <c:if test="${pxsq.status==10&&pxsq.isregister==1}">
										  <a href="<%=path%>/returntrainsheet.action?number=${pxsq.number}&newnumber=${newnumber}">反馈签到表</a>
										  </c:if>
									      <c:if test="${pxsq.status==9}">
										 	<a href="<%=path%>/showscoredetail.action?number=${pxsq.number}&newnumber=${newnumber}">开始评估</a>
										  </c:if>
										</div></td>	
							</tr>
							</c:forEach>
							...
							<td colspan="14">
								<div align="center">
								<a	href="<%=path%>/viewtrainapplyunder.action?newnumber=${newnumber}&currentPage=${previousPage}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/viewtrainapplyunder.action?newnumber=${newnumber}&currentPage=${nextPage}"
